/* MYSQL에서도 프로그래밍이 가능함.
JAVA, PYTHON 등과 달리 제약이 많지만 반복 작업을 줄여줄 수 있다는 점에서 많이 사용함 */

/* <MySQL에서 변수 사용하기>
변수를 지정할 때는 SET @변수명 = 값;의 문법을 사용합니다. */
SET @myVar1 = 5;
SET @myVar2 = 3;
SET @myVar3 = 4.25;
SET @myVar4 = '유저명 : ';
SET myVar5 = 50; -- 변수명 왼쪽에 @를 작성 안하면...? 저장이 안 된다.
...
/* PREPARE 구문
PREPARE 구문에 가변적으로 입력되는 문장요소의 자리에 ?를 입력하고, 그 자리를 EXCUTE 구문으로 채움
PREPARE 구문이름 '실제 쿼리문'; 으로 선언
EXECUTE 구문이름 USING 전달 변수; 로 호출 */
SET @myVar5 = 3;
PREPARE myQuery -- myQuery라는 이름으로 쿼리를 준비한다.
...
-- 프로시저 삭제 시 DROP PROCEDURE 프로시저명; -- ()는 쓰지 않습니다!
...
BEGIN 
    ...
    IF(days/365) >= 3 THEN
		SELECT CONCAT('가입한 지 ', days, '일이 경과했습니다.');
	ELSE 
...
/* 프로시저 getThree()의 문제점은 조회하려는 회원번호를 바꿀 때마다
...
SELECT @msg;
-- 게임 아무거나 하나 삭제하기
DELETE FROM game_list WHERE id = 4;
-- 데이터 삭제 후
SELECT @msg;