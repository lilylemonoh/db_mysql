...
use DB명; 을 수행해 호출할 수도 있습니다.*/

-- Workbench(윈도우)에서 수행가능한 구문은 거의 모두 CLI 환경에서 수행 가능합니다.
-- DATABASE 변경구문;
...
/* 특정 테이블은 원래 조회할 때 SELECT * FROM 데이터베이스명.테이블명; 형식으로 조회해야 합니다.
그러나 use 구문 등을 이용해 데이터베이스를 지정한 경우는 데이터베이스를 생략할 수 있습니다. */
SELECT * FROM bitcamp06.user_tbl;
SELECT * FROM user_tbl;
			-- AUTO_INCREMENT가 걸린 컬럼은 null을 주면 알아서 숫자 배정
...
-- 주의! WHERE를 걸지 않으면 해당 컬럼의 모든 값을 다 통일시켜 버립니다.
...
VALUES(값1, 값2, 값3 ...),
		...
		(값7, 값8, 값9 ...),
        ...; 
        컬럼명은 모든 컬럼에 값을 집어넣을 시 생략 가능합니다*/
...
SELECT * FROM user_tbl WHERE user_birth_year > 1995; -- SELECT문 하면 어떤 게 INSERT되는지 미리 확인할 수 있음

-- 다른 조건으로 한번 복습해 보았음
-- INSERT INTO user_tbl2
--	SELECT * FROM user_tbl
--    WHERE user_height > 170;
...
-- 두 번째 테이블인 구매내역을 나타내는 buy_tbl을 생성해 보겠습니다.
...
-- 만약 추가적인 설정 없이 user-tbl의 4번 유저를 삭제하고 싶다면
-- 먼저 buy-tbl의 4번유저가 남긴 구매내역을 모두 삭제해야 합니다.
...
-- DELETE FROM을 활용해서 user_tbl2의 2020-08-15일 이후 가입자를 삭제해 보세요.
-- safe모드 풀거나 CLI에서 진행하면 됨
...
-- ,를 이용해서 두 개 이상의 컬럼 이름을 한번에 바꿀 수 있습니다.
