...
-- JOIN할 컬럼 명이 동일하다면, ON 대신 USING(공통 컬럼명)구문을 대신 써도 됩니다.
...
-- 그래서 결과 ROW의 개수는 테이블1의 ROW 개수 * 테이블2의 ROW 개수가 됩니다.

-- 테스트 코드를 돌려보고, 예제 테이블을 다시 만들어보겠습니다.
SELECT * FROM 
	user_tbl, buy_tbl; -- ,로 나열 시 cross join 구문 생략 가능
-- user_tbl 10개 로우, buy_tbl은 3개로우 = 크로스조인 결과 로우는 30개
SELECT COUNT(*) FROM
	user_tbl CROSS JOIN buy_tbl;
    
SELECT u.user_num, b.buy_num
FROM user_tbl u, buy_tbl b;
...
-- SELF조인은 자기 테이블 내부 자료를 참조하는 컬럼이 있을 때
...
-- SELF JOIN을 이용해 직원이름과 상사이름이 같이 나오게 만들어 보겠습니다.
-- SELF JOIN은 테이블 자기 자신을 자기가 참조하도록 만들기 때문에
-- JOIN 시 테이블명은 좌, 우 같은 이름으로, AS를 이용해서 부여한 별칭은 다르게 해서
-- 좌측과 우측 테이블을 구분하면 됩니다. 이외에는 모두 같습니다.

SELECT l.staff_num, l.staff_name as 상급자이름, r.staff_name as 하급자이름
