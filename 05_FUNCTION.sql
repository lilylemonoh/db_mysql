...
-- 문자열 함수를 활용해서, 하나의 컬럼을 여러 형식으로 조회해 보겠습니다.
SELECT 
	user_name,
...
-- LENGTH는 byte 길이이므로 한글은 한 글자에 3바이트로 간주합니다.
...
-- user_tbl에 소수점 아래를 저장받을 수 있는 컬럼을 추가해보겠습니다.
-- DECIMAL은 고정자리수이므로 반드시 소수점 아래 2자리까지 표시해야 합니다.
-- 전체 3자리 중 소수점 아래에 두 자리를 배정하겠다. -> 5자리 중 2자리로 수정함
ALTER TABLE user_tbl ADD (user_weight DECIMAL(3, 2));
ALTER TABLE user_tbl MODIFY user_weight DECIMAL(5, 2);

SELECT * FROM user_tbl;

UPDATE user_tbl SET user_weight = 55.52 WHERE user_num = 6;

-- 숫자 함수 사용 예제
SELECT
	user_name, user_weight,
    ROUND(user_weight, 0) AS 무게반올림,
    TRUNCATE(user_weight, 0) AS 소수점아래_1자리절사,
    MOD(user_height, 150) AS 키_150으로나눈나머지,
    CEIL(user_weight) AS 무게올림,
    FLOOR(user_weight) AS 무게내림,
...
SELECT
    ...
    DATE_ADD(entry_date, INTERVAL 15 DAY) AS _15일후,
...
-- user_height, user_weight이 NULL인 자료를 추가해 보겠습니다.
...
-- null을 0으로 바꾸면 평균값 구할 때 왜곡이 일어날 수 있기 때문에
-- 평균값을 넣거나 평균값도 왜곡이 될 수 있을 때 중위값을 넣어서 값을 보정할 수 있음.
SELECT 
	user_name, ifnull(user_height, 167) AS _NULL대체값넣은키, ifnull(user_weight, 65) AS _NULL대체값넣은체중
FROM
    user_tbl;
    
-- SQL에서 0으로 나누면 무슨 일이 벌어지는지 보겠습니다.
SELECT 3/0;

-- user_tbl의 회원들 중 키 기준으로 
-- 165 미만은 평균 미만, 165는 평균, 165이상은 평균이상으로 출력해보겠습니다.
SELECT 
	user_name,
    user_height,
    CASE 
...
-- BMI지수의 결과를 18 미만이면 '저체중', 18~24면 '일반체중', 25 이상이면 '고체중'으로 
-- CASE 구문을 이용해서 출력해주세요. 
-- BMI 지수를 나타내는 컬럼은 BMI_RESULT이고, BMI 분류를 나타내는 컬럼은 BMI_CASE입니다.

SELECT --                                                  power(user_height/100, 2)
	user_name, user_height, user_weight, user_weight/((user_height/100)*(user_height/100)) AS BMI_RESULT,
    CASE 
        ...
        WHEN  user_weight/((user_height/100)*(user_height/100)) >= 25 THEN '고체중'
	END AS BMI_CASE
FROM user_tbl;
