...
-- FROM절 다음에는 테이블명만 올 수 있는 게 아니고, 데이터 시트형식을 갖춘 자료라면 
-- 모든 형태의 자료를 적어줄 수 있습니다.
-- user_tbl 중 키가 170 미만인 요소만 1차적으로 조회한 결과시트를 토대로 전체데이터 조회 구문
-- 서브쿼리를 FROM절에서 사용할 때는 별칭을 무조건 붙여야 합니다. 
SELECT A.* FROM (SELECT * FROM user_tbl WHERE user_height < 170) A;

-- 서브쿼리의 실제 활용 : 최지선보다 키가 큰 사람을 한 줄로 결과 얻기.
-- 기존 방식으로 처리하는 경우
-- 1. 최지선의 키를 WHERE 절을 이용해서 확인.
...
SELECT * 
FROM user_tbl
WHERE user_height > (SELECT user_height FROM user_tbl WHERE user_name = '최지선');

-- 위 코드를 참조해서, 평균보다 체증이 낮은 사람만 조회하는 쿼리문을 서브쿼리를 활용하여 만들어주세요.
SELECT *
FROM user_tbl
WHERE user_weight < (SELECT AVG(user_weight) FROM user_tbl);

-- 지역이 경기인 사람들 중 키가 제일 큰 사람보다 키가 더 큰 사람을 조회해 주세요.
SELECT * 
FROM user_tbl
